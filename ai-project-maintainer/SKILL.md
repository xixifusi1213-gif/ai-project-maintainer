...
6. For a reusable local safety gate, run `node <this-skill>/scripts/run-local-gate.mjs <repo> --agent-risk --strict --release --output reports/security-report.json` or `npx ai-project-maintainer gate <repo> --agent-risk --strict --release --output reports/security-report.json`.
7. For production evidence review, run `node <this-skill>/scripts/run-local-gate.mjs <repo> --production --agent-risk --strict --release --output reports/security-report.json` or `npx ai-project-maintainer gate <repo> --production --agent-risk --strict --release --output reports/security-report.json`.
