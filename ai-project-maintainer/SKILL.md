...
- **AI agent repair pack**: After a gate run, generate `reports/agent-tasks.json`, `reports/codex-tasks.json`, `reports/fix-plan.md`, and recheck scripts with `repair-pack`. Treat `agent-tasks.json` as the generic source of truth for AI coding assistants. Codex may use `codex-tasks.json`, but do not assume the user only uses Codex.
