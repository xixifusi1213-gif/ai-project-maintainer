...
import { resolveSpawnCommand, resolveSpawnTarget, verifyPrepublish, verifyPublished } from "../ai-project-maintainer/scripts/verify-release.mjs";
...
test("published verification resolves npm command for Windows without shell execution", () => {
  ...
  const npmTarget = resolveSpawnTarget("npm", ["--version"], "win32", { ComSpec: "C:\\Windows\\System32\\cmd.exe" });
  assert.deepEqual(npmTarget, {
    command: "C:\\Windows\\System32\\cmd.exe",
    args: ["/d", "/s", "/c", "call", "npm.cmd", "--version"],
  });
  assert.deepEqual(resolveSpawnTarget("git", ["--version"], "win32"), {
    command: "git",
    args: ["--version"],
  });
});
...
test("publish workflow uses OIDC trusted publishing and avoids long-lived npm tokens", () => {
  ...
  assert.match(workflowText, /--commit "\$\(git rev-parse HEAD\)"/);
  assert.match(workflowText, /attest-build-provenance/);
  assert.match(workflowText, /grep -m 1 '\^# '/);
  assert.doesNotMatch(workflowText, /Trusted release chain/);
});
